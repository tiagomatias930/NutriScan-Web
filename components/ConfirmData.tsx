...
interface ConfirmDataProps {
  googleName: string;
  googleEmail: string;
  onConfirm: (data: Partial<UserProfile>) => void;
  onSkip: () => void;
}
...
export const ConfirmData: React.FC<ConfirmDataProps> = ({
  googleName,
  googleEmail,
  onConfirm,
  onSkip
  ...
  return (
    <div className="min-h-screen bg-dark text-textLight flex flex-col p-6 font-sans relative overflow-hidden">
      ...
      <div className="flex-1 flex flex-col max-w-md mx-auto w-full py-8 relative z-10">
        ...
        {/* Step 1: Confirm Name & Gender from Google */}
        {step === 1 && (
          <div className="animate-fade-in flex-1">
            <div className="flex items-center gap-3 mb-6">
              ...
              <div>
                <h2 className="text-xl font-bold text-textLight">{t('confirm.title')}</h2>
                <p className="text-sm text-textMuted">{t('confirm.subtitle')}</p>
              </div>
            </div>

            {/* Google Account Info */}
            <div className="glass-lg rounded-2xl p-4 mb-6">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-full bg-primary/30 flex items-center justify-center">
                  <span className="material-icons text-primary">account_circle</span>
                </div>
                <div className="flex-1">
                  <p className="text-sm text-textMuted">{t('confirm.loggedAs')}</p>
                  <p className="text-textLight font-medium">{googleEmail}</p>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-6">
              <div>
                <label className="text-sm font-medium text-textMuted mb-2 block ml-1">
                  {t('confirm.nameLabel')}
                </label>
                <input
                  type="text"
                  className="w-full glass-lg text-textLight border-primary/20 focus:border-primary rounded-2xl p-5 text-lg focus:outline-none focus:ring-2 focus:ring-primary transition-all"
                  value={data.name}
              ...
              <div>
                <label className="text-sm font-medium text-textMuted mb-3 block ml-1">
                  {t('confirm.genderLabel')}
                </label>
              ...
              {SOMATOTYPE_OPTIONS.map(item => (
                ...
                >
                  <div className={`w-12 h-12 rounded-2xl flex items-center justify-center flex-shrink-0 transition-colors ${
                    data.somatotype === item.value ? 'bg-primary text-white' : 'bg-glassMedium text-textMuted'
                  }`}>
                    <span className="material-icons">{item.icon}</span>
                  </div>
                  <div>
                    <div className={`font-bold text-lg mb-1 ${data.somatotype === item.value ? 'text-primary' : 'text-textLight'}`}>
                      {t(item.titleKey)}
                    </div>
                    <div className="text-sm text-textMuted leading-relaxed">{t(item.descriptionKey)}</div>
                  </div>
                </button>
              ))}
            </div>
        ...
        {step === 4 && (
          <div className="animate-fade-in flex-1">
            ...
            <div>
              <label className="text-sm font-medium text-textMuted mb-3 block ml-1">
                {t('onboarding.step4.activityLabel')}
              </label>
              <div className="relative">
                <select 
                  value={data.activityLevel}
                  onChange={e => updateData('activityLevel', e.target.value as ActivityLevel)}
                  className="w-full glass-lg text-textLight p-5 rounded-2xl border border-glassDark focus:border-primary outline-none appearance-none text-lg"
                >
                  {ACTIVITY_OPTIONS.map(({ value, labelKey }) => (
                    <option key={value} value={value} className="bg-dark text-textLight">{t(labelKey)}</option>
                  ))}
                </select>
                <div className="absolute right-5 top-1/2 transform -translate-y-1/2 pointer-events-none text-textMuted">
        ...
        {/* Navigation Buttons */}
        <div className="pt-6 pb-4 flex gap-3">
          {step > 1 && (
            <button 
              onClick={() => setStep(s => s - 1)}
              className="flex-1 py-4 glass text-textLight font-bold text-lg rounded-2xl hover:glass-lg transition-all"
            >
          ...
          {step < 4 ? (
            <button 
              ...
              className="flex-1 py-4 bg-primary text-black font-bold text-lg rounded-2xl disabled:opacity-50 disabled:cursor-not-allowed hover:bg-emerald-400 transition-colors shadow-lg shadow-emerald-900/20"
            >
            ...
            <button 
              ...
              className="flex-1 py-4 bg-gradient-to-r from-primary to-primaryDark text-white font-bold text-lg rounded-2xl hover:opacity-90 transition-opacity shadow-lg shadow-emerald-900/30 flex items-center justify-center gap-2"
            >
...
const StatSlider = ({ label, value, min, max, unit, onChange }: { label: string, value: number, min: number, max: number, unit: string, onChange: (v: number) => void }) => (
  <div className="glass-lg p-5 rounded-3xl">
    <div className="flex justify-between mb-4 items-end">
      <span className="text-textMuted font-medium">{label}</span>
      <div className="flex items-baseline gap-1">
        <span className="font-bold text-2xl text-textLight">{value}</span>
        <span className="text-sm text-textMuted font-medium">{unit}</span>
      </div>
    </div>
    <input 
